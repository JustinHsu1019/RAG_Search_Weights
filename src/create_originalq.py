...
def process_questions(input_file, output_file):
    ...
    filtered = [item for item in data if item.get('tagone') == 'questionWithKeyword']
    ...
    for idx, item in enumerate(filtered, start=1):
        ...
        # 使用空格分割問題，取第一個部分
        first_part = question_processed.split(' ')[0]
        
        # 生成新的 qid，格式為 parentqid_originalquestion_X
        new_qid = f"{parentqid}_originalquestion_{idx}"
...
if __name__ == "__main__":
    ...
    output_path = 'data/question_original.json'
